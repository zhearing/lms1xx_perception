...
{
  if(!listener.waitForTransform( front_scan->header.frame_id, "/base", front_scan->header.stamp + ros::Duration().fromSec(front_scan->ranges.size()*front_scan->time_increment), ros::Duration(1.0)))
  {
     return;
...
{
  if(!listener.waitForTransform( left_scan->header.frame_id, "/base", left_scan->header.stamp + ros::Duration().fromSec(left_scan->ranges.size()*left_scan->time_increment), ros::Duration(1.0)))
  {
     return;
...
void LaserToCloud::right_scanner_callback(const sensor_msgs::LaserScan::ConstPtr& right_scan)
{
  ...
  if(!listener.waitForTransform( right_scan->header.frame_id, "/base", right_scan->header.stamp + ros::Duration().fromSec(right_scan->ranges.size()*right_scan->time_increment), ros::Duration(1.0)))
  {
     return;
...
void LaserToCloud::rear_scanner_callback(const sensor_msgs::LaserScan::ConstPtr& rear_scan)
{
  ...
  if(!listener.waitForTransform( rear_scan->header.frame_id, "/base", rear_scan->header.stamp + ros::Duration().fromSec(rear_scan->ranges.size()*rear_scan->time_increment), ros::Duration(1.0)))
  {
     return;
