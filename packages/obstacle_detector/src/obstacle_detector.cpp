...
ObstacleDetector::ObstacleDetector() : nh_(""), nh_local_("~") {
    ...
    if (p_use_scan_)
        scan_sub_ = nh_.subscribe(p_laser_topic_, 10, &ObstacleDetector::scanCallback, this);
    else if (p_use_pcl_)
        pcl_sub_ = nh_.subscribe(p_pc_topic_, 10, &ObstacleDetector::pclCallback, this);
...
void ObstacleDetector::updateParams() {
    nh_local_.param<std::string>("laser_topic", p_laser_topic_, "front_scan");
    nh_local_.param<std::string>("pc_topic", p_pc_topic_, "pcl");
    nh_local_.param<std::string>("world_frame", p_world_frame_, "base");
    nh_local_.param<std::string>("scanner_frame", p_scanner_frame_, "tramcar");
...
void ObstacleDetector::groupPointsAndDetectSegments() {
    ...
    for (const Point &point : initial_points_) {
        if (point_set.size() != 0) {
            ...
            if ((point - point_set.back()).lengthSquared() >
                pow(p_max_group_distance_ + r * p_distance_proportion_, 2.0)) {
